"""Tiny-VAE decoder loading/serving, SELF-CONTAINED copy for offline H3 preview.

SOURCE: vendored from kijai / "ComfyUI-KJNodes", file `nodes/tiny_vae.py`
(https://github.com/kijai/ComfyUI-KJNodes). Copied so the MMH3 preview node does
not require KJNodes to be installed. Keep this module in sync with upstream when
updating. Attribution: kijai / KjNodes contributors (MIT-style, see upstream).

The 2D taeh3 checkpoint is a flattened TAESD-style decoder (96-wide, 4 upsamples)
that core ComfyUI can't build (its Decoder hardcodes a 64-wide / 3-upsample
stack); TinyVAEDecoder rebuilds the architecture from positional key indices.
The temporal taehv format (taeh3_H3, 24 latent channels) is built by core's TAEHV
but core can't size it for H3; TAEHVDecoder patches the patch-dependent convs.

`load_tiny_vae_decoder(name)` is the single entry point: it inspects the state
dict and returns the right decoder, or None (with a log) if unusable.
"""
...
class TinyVAEDecoder:
    ...
    def __init__(self, sd, device=None, dtype=None):
        ...
        self.dtype = dtype if dtype is not None else _vae_dtype_safe(
...
def _vae_dtype_safe(device, allowed):
    """vae_dtype() but CPU-safe: ComfyUI's should_use_fp16() calls
    torch.cuda.get_device_properties() and blows up on a CPU device, so fall back
    to fp32 there instead of letting it raise."""
    try:
        if torch.device(device).type == "cpu":
            return torch.float32
    except Exception:
        pass
    return comfy.model_management.vae_dtype(device, allowed)
